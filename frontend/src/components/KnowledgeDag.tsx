...
function buildLayout(points: KnowledgePoint[], relations: KnowledgeRelation[]): DagLayout {
  ...
  const columns = new Map<number, KnowledgePoint[]>()
  for (const point of points) {
    const pointDepth = depth.get(point.pointId) || 0
    columns.set(pointDepth, [...(columns.get(pointDepth) || []), point])
  }
  for (const column of columns.values()) column.sort((a, b) => a.title.localeCompare(b.title, 'zh-CN'))

  const maximumDepth = Math.max(0, ...columns.keys())
  const maximumRows = Math.max(1, ...[...columns.values()].map((column) => column.length))
  const width = CANVAS_PADDING * 2 + (maximumDepth + 1) * NODE_WIDTH + maximumDepth * COLUMN_GAP
  const height = Math.max(420, CANVAS_PADDING * 2 + maximumRows * NODE_HEIGHT + (maximumRows - 1) * ROW_GAP)
  ...
  for (const [columnDepth, column] of columns) {
    ...
    column.forEach((point, index) => nodes.push({
      point,
      x: CANVAS_PADDING + columnDepth * (NODE_WIDTH + COLUMN_GAP),
...
export default function KnowledgeDag({ points, relations, selectedPointId, onSelect }: {
  ...
  const layout = useMemo(() => buildLayout(points, relations), [points, relations])
  ...
  function zoomAt(nextScale: number, clientX?: number, clientY?: number) {
    ...
    setTransform((current) => {
      const scale = clamp(nextScale, .05, 2)
