...
项目仍在持续开发中。当前 RenderService 已可向浏览器提供运行时清单、`adapter.js` 和 `runtime.wasm`；完整 ReviewSession 执行引擎尚未实现。GalGameService 当前使用进程内临时存储，适合开发联调，不应视为生产级持久化方案。OCRService 为可选内部依赖，不属于当前最小闭环的必启服务。
...
- MySQL 8：本地开发配置目前使用 `127.0.0.1:3306`。
- MongoDB：本地开发配置目前使用 `127.0.0.1:27017`。
