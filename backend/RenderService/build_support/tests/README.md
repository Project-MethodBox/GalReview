...
| rust_validate | `languages/rust/modules/validate.lua`(单 crate:root_file+sources+export_prefix 显式驱动) | 健康 crate 树校验干净;`// rust.cargo: allow-orphan` 逃生口生效;文件模块(`name.rs`)在 `name/` 下的子模块可达(现代布局);ABI 白名单豁免 `rust_eh_personality`;未声明前缀时跳过前缀校验 | mod.rs 孤儿(建议行按目录名)、缺 `#![no_std]`、`#[unsafe(no_mangle)]` 无项目声明前缀 |
...
| rust_archive | `languages/rust/modules/archive.lua`(原生 ar 成员读取)+ `cargo.lua`(`unpack_objects` staticlib 摊平) | GNU(`//` 长名表)/COFF(NUL 终止表+双符号表)/BSD(`#1/<n>` 内联名)三方言字节保真解析;非对象成员(符号表/rmeta)过滤;摊平命名 `rust-NNNN-NNNN.o[bj]` 与签名 marker 缓存/失效;旧 `ar x` 布局残留清除;84 字符成员名×深输出目录不越 MAX_PATH(取代外部 `ar x` 的动机回归) | thin 归档、坏魔数、损坏成员头、截断成员、无名字表的长名引用、零对象成员 staticlib、owner 之外的输出目录,全部响亮失败 |
